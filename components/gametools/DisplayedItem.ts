...
type StyleEntry = { style: Style; selector: string; styleSheet?: StyleSheet };
...
export abstract class DisplayedItem {
    ...
    jssStyles: StyleEntry[];
    protected getJssStyle(style: Style): string {
        let selector = null;
        this.jssStyles.some((entry) => {
        ...
        if(selector == null ) {
            const jss = GameTools.assertProperty(window, "jss");
            const styleSheet = jss.createStyleSheet({ "displayedItemStyle" : style });
            styleSheet.attach();
            selector = styleSheet.getRule("displayedItemStyle").selector;
            this.jssStyles.push({ style: style, selector: selector, styleSheet: styleSheet });
        }
    ...
    async _undisplay() {
        this.jssStyles.forEach((entry) => {
            entry.styleSheet.detach();
        });
        this.jssStyles = [];
